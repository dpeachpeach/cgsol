...
class Poller:
    ...
    async def reconcile(self, full: bool | None = None) -> None:
        ...
        # Skew is only known once a response has carried a `Date`, so the very
        # first read above is on our clock. Never let an unsynced one carry the
        # window past what GitHub says the time is, or the gap goes unswept.
        swept_at = min(swept_at, self.github.server_time())
